...
public class PlatformLinuxGLCanvas implements PlatformGLCanvas {
    ...
    public long create(Canvas canvas, GLData attribs, GLData effective) throws AWTException {
        if (ds == null) {
            ...
            if (ds == null) {
                throw new AWTException("AWT_GetDrawingSurface() failed");
            }
        }
        ...
        try {
            ...
            if (dsi == null) {
                throw new AWTException("JAWT_DrawingSurface_GetDrawingSurfaceInfo() failed");
            }
            
            try {
                JAWTX11DrawingSurfaceInfo dsi_x11 = JAWTX11DrawingSurfaceInfo.create(dsi.platformInfo());
                return create(dsi_x11, attribs, effective);
            } finally {
    ...
    public boolean deleteContext(long context) {
        ...
        try {
            JAWTX11DrawingSurfaceInfo dsi_x11 = JAWTX11DrawingSurfaceInfo.create(dsi.platformInfo());
            glXDestroyContext(dsi_x11.display(), context);
            return true;
        } finally {
            JAWT_DrawingSurface_FreeDrawingSurfaceInfo(dsi, ds.FreeDrawingSurfaceInfo());
        }
    ...
    private static void populateEffectiveGLAttribs(GLData effective) throws AWTException {
        ...
        if (version.major >= 3) {
            if (version.major >= 4 || version.minor >= 2) {
                ...
                } else if ((profileFlags & GLX_CONTEXT_ES_PROFILE_BIT_EXT) != 0) {
